...
impl Payload for CompositePayload {
    ...
    fn max_soft_bundle_size(&self) -> std::num::NonZeroUsize {
    ...
    async fn make_transaction_batch(&mut self) -> Vec<Transaction> {
        ...
        for (i, gas) in current_batch_gas.iter().take(batch_size).enumerate() {
            ...
            {
                used_gas.push(i);
                // Build a gas payment mixing 1-2 real coins with 1-2 coin-reservation digest
                // entries, then shuffle the whole vector. Exercises the gas-smashing path
                // that the IFFW settlement-underflow bug lives on: when the tx gets cancelled
                // with IFFW, any address-balance entry left in the payment list contributes a
                // Split against the (now possibly depleted) accumulator at settlement time.
                ...
                // Base reservation in the same order of magnitude as the AB amount: large
                // enough that concurrent AB-overdraw txs leave it underfunded (so IFFW fires
                // on this tx), but not so absurdly large that signing or scheduling rejects
                // it outright. The second fake coin uses base+1 so it has a distinct digest.
